...
def train_net(net, device, opt, logger):
    ...
    def compute_allI(task_id):
        ...
        with torch.no_grad():
            for i in range(l):
                All_Importance[i] = (All_Importance[i] * task_id + best_Importance[i]) / (task_id + 1)
    ...
    for epoch in epochs_range:
        ...
        for image, label in loader_train:
            ...
            if opt.task_id > 0:
                for i, w in enumerate(net.parameters()):
                    tmp += (opt.beta / 2 * torch.sum(torch.mul(All_Importance[i], torch.abs(w - old_Star_vals[i])))
                            + opt.beta / 2 * torch.sum(
                                torch.square(torch.mul(All_Importance[i], torch.abs(w - old_Star_vals[i])))))
            loss += tmp
            total_loss += loss.data.cpu().numpy().item()
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method_name', type=str, default='PIGWM-SGD', help='Method name')
    ...
    # train_exp = ['PIGWM', 'PIGWM-tiny', 'PIGWM-small', 'PIGWM-large']
