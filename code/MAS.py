...
def train_net(net, device, opt, logger):
    ...
    def compute_I(image, batch_num):
        net.eval()  ##防止bn层参数发生改变
        pred = net(image).softmax(dim=1)
        Label_zeros = torch.zeros_like(pred)
        diff = torch.nn.MSELoss(size_average=False)
        loss = diff(pred,Label_zeros)
        net.zero_grad()
        optimizer.zero_grad()
        loss.backward()
        # pred = net(image).softmax(dim=1)
        # l2 = torch.norm(pred)
        # loss = torch.square(l2)
        # net.zero_grad()
        # optimizer.zero_grad()
        # loss.backward()
        with torch.no_grad():
    ...
    def compute_allI():
        ...
        imp_min = 1e20
        for i in range(l):
            imp_max = max(imp_max, torch.max(best_Importance[i]))
            imp_min = min(imp_min, torch.min(best_Importance[i]))
        for i in range(l):
            best_Importance[i] = (best_Importance[i] - imp_min) / (imp_max - imp_min)
        with torch.no_grad():
            for i in range(l):
                All_Importance[i] = (All_Importance[i] + best_Importance[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method_name', type=str, default='MAS-SGD', help='Method name')
    ...
    # train_exp = ['MAS', 'MAS-tiny', 'MAS-small', 'MAS-large']
