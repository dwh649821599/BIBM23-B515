...
def train_net(net, device, opt, logger):
    ...
    if opt.task_id > 0:
        ...
        for kk in range(opt.task_id):
            if kk == 0:
                path = os.path.join(opt.save_path, 'task{}'.format(kk))
            else:
                path = os.path.join(opt.save_path, 'task{}_{}'.format(kk, opt.beta))

            All_Importances.append(torch.load(os.path.join(path, 'All_Importance.pth')))
            All_Star_vals.append(torch.load(os.path.join(path, 'Star.pth')))

            if kk == (opt.task_id - 1):
                net.load_state_dict(torch.load(os.path.join(path, 'net_best.pth')))
    else:
        print('First task...')
    All_Importance = []
    Star_vals = []
    ...
    def compute_I(batch_num):
        with torch.no_grad():
            for i, w in enumerate(net.parameters()):
                ...
                Importance[i].add_(torch.square(w.grad.data) / (batch_num + 1))

    def compute_allI(task_id):
        l = len(best_Importance)
        with torch.no_grad():
            for i in range(l):
                All_Importance[i] = best_Importance[i]
    ...
    def create_npy():
        ...
        if opt.task_id == 0:
            ...
            net.load_state_dict(
                torch.load(os.path.join(opt.save_path, f'task{opt.task_id}_{int(opt.beta)}', 'net_best.pth')))
        with torch.no_grad():
            for i, w in enumerate(net.parameters()):
    ...
    for epoch in epochs_range:
        ...
        for image, label in loader_train:
            ...
            if opt.task_id > 0:
                for kk in range(len(All_Importances)):
                    for i, w in enumerate(net.parameters()):
                        tmp += opt.beta / 2 * torch.sum(
                            torch.mul(All_Importances[kk][i], torch.square(w - All_Star_vals[kk][i])))
            loss += tmp
            total_loss += loss.data.cpu().numpy().item()
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method_name', type=str, default='EWC-SGD', help='Method name')
    parser.add_argument('--order', type=int, default=1, help='Training order')
    ...
    parser.add_argument('--save_path', type=str, default='../parameters/PIGWM', help='Path to save models')
    ...
    # train_exp = ['EWC', 'EWC-tiny', 'EWC-small', 'EWC-large']
