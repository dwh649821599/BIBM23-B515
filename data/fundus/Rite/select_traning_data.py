...
root = './'
all_packets = glob.glob(os.path.join(root, 'image', '*.png'))
for i, packet in enumerate(all_packets):
    all_packets[i] = os.path.basename(packet).replace('.png', '')
...
train_flag = random.sample(range(0, 100), 80)
val_flag = random.sample(train_flag, 20)
test_flag = list(set(range(0, 100)) - set(train_flag))
